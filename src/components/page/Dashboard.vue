                ...
                <div class="userInfo">
                    <div class="user-info">
                        ...
                        <div class="user-info-cont">
                            ...
                            <div>{{LogingType==3?'调度员':'客服'}}</div>
                        </div>
                    </div>
                    <div class="user-info-list">
                        上次登录时间：<span>{{userInfo.llt}}</span>
                    </div>
                    <div class="user-info-list">
                        上次登录IP：<span>{{userInfo.lli}}</span>
                    </div>
                </div>
				<!--  -->
				<div class="marketBox">
...
export default {
    ...
    created() {
		...
		if(this.LogingType==3){
			this.userInfo = this.loginData.DispatchData;
		}else if(this.LogingType==4){
			this.userInfo = this.loginData.CustomerData;
    ...
    methods: {
		...
		async orderStatistics(type){
			...
			// return;
            let res = await request('/32004', hd, bd);
			let data = '';
		...
		async getTodayData(){
			...
			let res = await request('/32003', hd, bd);
			let data = '';
			if (this.userInfo.em == 0) {
				data = JSON.parse(res.data.bd);
			}else if(this.userInfo.em == 1) {
			...
			for(let i=0; i<3; i++){
				let title = '维护客户数量';
				let _val = data.totalCustom;
				let color = '#7ecf51';
				if( i==1 ){
					title = '累计订单数量';
					_val = data.totalOrder;
					color = '#2d8cf0';
				}else if( i==2 ){
					title = '累计运单数量';
					_val = data.totalBill;
